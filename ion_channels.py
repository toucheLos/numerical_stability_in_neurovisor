...
def expm1_div(x_num, x_den):
    ...
    mask = np.abs(d) > 1e-14
    out[mask] = x_num[mask] / d[mask]
    # series approx for small |x_den|
    xs = x_den[~mask]
    out[~mask] = 1.0 - 0.5*xs + (xs**2)/12.0
    return out
...
def k_rates(V, vT_mV=V_SHIFTS['vT']):
    ...
    vT = float(vT_mV)
    x = (15.0 + vT - Vm)          # mV
    a = 1.0e3 * 0.032 * expm1_div(x, x/5.0)
...
def na_rates(V, vT_mV=V_SHIFTS['vT']):
    ...
    xm = (13.0 + vT - Vm)
    am = 1.0e3 * 0.32 * expm1_div(xm, xm/4.0)

    bm_x = (Vm - vT - 40.0)
    bm = 1.0e3 * 0.28 * expm1_div(bm_x, bm_x/5.0)
