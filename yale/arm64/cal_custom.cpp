...
#define nrn_init _nrn_init__cal_custom
#define _nrn_initial _nrn_initial__cal_custom
#define nrn_cur _nrn_cur__cal_custom
#define _nrn_current _nrn_current__cal_custom
#define nrn_jacob _nrn_jacob__cal_custom
#define nrn_state _nrn_state__cal_custom
#define _net_receive _net_receive__cal_custom 
#define rate rate__cal_custom 
#define states states__cal_custom 
...
#define tauq _ml->template fpfield<3>(_iml)
#define tauq_columnindex 3
#define rinf _ml->template fpfield<4>(_iml)
#define rinf_columnindex 4
#define taur _ml->template fpfield<5>(_iml)
#define taur_columnindex 5
#define q _ml->template fpfield<6>(_iml)
#define q_columnindex 6
#define r _ml->template fpfield<7>(_iml)
#define r_columnindex 7
#define eca _ml->template fpfield<8>(_iml)
#define eca_columnindex 8
#define Dq _ml->template fpfield<9>(_iml)
#define Dq_columnindex 9
#define Dr _ml->template fpfield<10>(_iml)
...
static void register_nmodl_text_and_filename(int mechtype);
 ...
 {"setdata_cal_custom", _hoc_setdata},
 {"rate_cal_custom", _hoc_rate},
 {0, 0}
};
...
static NPyDirectMechFunc npy_direct_func_proc[] = {
 ...
 {"gcalbar_cal_custom", "S/cm2"},
 {"ica_cal_custom", "mA/cm2"},
 {"tauq_cal_custom", "ms"},
 {"taur_cal_custom", "ms"},
 {0, 0}
};
 ...
 hoc_register_parm_default(_mechtype, &_parm_default);
  ...
  register_nmodl_text_and_filename(_mechtype);
   ...
   _nrn_mechanism_register_data_fields(_mechtype,
                                       ...
                                       _nrn_mechanism_field<double>{"tauq"} /* 3 */,
                                       _nrn_mechanism_field<double>{"rinf"} /* 4 */,
                                       _nrn_mechanism_field<double>{"taur"} /* 5 */,
                                       _nrn_mechanism_field<double>{"q"} /* 6 */,
                                       _nrn_mechanism_field<double>{"r"} /* 7 */,
                                       _nrn_mechanism_field<double>{"eca"} /* 8 */,
                                       _nrn_mechanism_field<double>{"Dq"} /* 9 */,
                                       _nrn_mechanism_field<double>{"Dr"} /* 10 */,
...
static int _reset;
static const char *modelname = "Custom Calcium Channel (cal_custom)";
...
static int  rate ( _internalthreadargsprotocomma_ double _lv ) {
   double _la_q , _lb_q , _la_r , _lb_r ;
 _la_q = 1e3 * 0.055 * ( 27.0 - _lv ) / ( exp ( ( - 27.0 - _lv ) / 3.8 ) - 1.0 ) ;
   _lb_q = 1e3 * 0.94 * exp ( ( - 75.0 - _lv ) / 17.0 ) ;
   _la_r = 1e3 * 0.000457 * exp ( ( - 13.0 - _lv ) / 50.0 ) ;
   _lb_r = 1e3 * 0.0065 / ( exp ( ( - 15.0 - _lv ) / 28.0 ) + 1.0 ) ;
   qinf = _la_q / ( _la_q + _lb_q ) ;
   tauq = 1.0 / ( _la_q + _lb_q ) ;
   rinf = _la_r / ( _la_r + _lb_r ) ;
   taur = 1.0 / ( _la_r + _lb_r ) ;
    return 0; }
...
static void _hoc_rate(void) {
 ...
 Datum* _ppvar; Datum* _thread; NrnThread* _nt;
  ...
  if(!_prop_id) {
    hoc_execerror("No data for rate_cal_custom. Requires prior call to setdata_cal_custom and that the specified mechanism instance still be in existence.", NULL);
  }
...
static void _initlists(){
 int _i; static int _first = 1;
  if (!_first) return;
 _slist1[0] = {q_columnindex, 0};  _dlist1[0] = {Dq_columnindex, 0};
...
static void register_nmodl_text_and_filename(int mech_type) {
  ...
  "    gating variables (q and r) using Hodgkin\n"
  "\n"
  "\n"
  "Huxley-style kinetics. The rate \n"
